...
class ModelUserPriceUploadVendor extends Model {
	...
	public function updateAsabellaProduct($data) {
		if(!$data['id']) return false;
	...
	public function vendorProductCharacters($data, $product_id, $search, $vendor) {
		...
		if($search){
			foreach($data as $p){
				...
				$this->db->query("
					UPDATE import_products_character SET
						param_value = '" . $this->db->escape($p['value']) . "'
					WHERE product_id = ".$product_id." AND vendor = '".$vendor."'   AND param_name = '".$p['name']."  '
			");
			}
		}else {
			foreach($data as $p){
				$this->db->query("
					INSERT INTO import_products_character SET
						param_name = '" . $this->db->escape($p['name']) . "',
						param_value = '" . $this->db->escape($p['value']) . "',
						vendor = '" . $vendor . "',
